...
def mc_inference_hetero_variational(fn, fn_std, fd, opt):
    ...
    if opt['method'] == 'cnn_heteroscedastic_variational' or \
       ...
       opt['method'] == 'cnn_heteroscedastic_variational_upsc_control':
        ...
        std_m = np.sqrt(np.abs(sum_out2 - 2 * mean * sum_out + no_samples * mean ** 2) / no_samples)
        std_d = 1. * fn_std.eval(feed_dict=fd)
    else:
        raise Exception('The specified method does not support MC inference.')
        mean = fn.eval(feed_dict=fd)
        std_m = None
        std_d = None
    return mean, std_m, std_d
...
def super_resolve_mcdropout_heterovariational(dt_lowres, opt):
    ...
    with tf.Session() as sess:
        ...
        dt_hires_std_m = np.zeros(dt_lowres.shape)  # model uncertainty
        dt_hires_std_m[:, :, :, 0] = dt_lowres[:, :, :, 0]

        dt_hires_std_d = np.zeros(dt_lowres.shape)  # data uncertainty
        dt_hires_std_d[:, :, :, 0] = dt_lowres[:, :, :, 0]
        ...
        for i, j, k in recon_indx:
            ...
            opatch_mean_ps, opatch_std_m_ps, opatch_std_d_ps = \
                mc_inference_hetero_variational(y_pred, y_pred_std, fd, opt)
            ...
            opatch_std_m = forward_periodic_shuffle(opatch_std_m_ps, upsampling_rate)
            dt_hires_std_m[upsampling_rate * (i - output_radius - 1):
                         upsampling_rate * (i + output_radius),
                         upsampling_rate * (j - output_radius - 1):
                         upsampling_rate * (j + output_radius),
                         upsampling_rate * (k - output_radius - 1):
            ...
            opatch_std_d = forward_periodic_shuffle(opatch_std_d_ps, upsampling_rate)
            dt_hires_std_d[upsampling_rate * (i - output_radius - 1):
            upsampling_rate * (i + output_radius),
            upsampling_rate * (j - output_radius - 1):
            upsampling_rate * (j + output_radius),
            upsampling_rate * (k - output_radius - 1):
        ...
        dt_hires_std_m = dt_trim(dt_hires_std_m, padding)
        dt_hires_std_d = dt_trim(dt_hires_std_d, padding)
        mask = dt_hires[:, :, :, 0] != -1
        dt_hires[..., 2:] = dt_hires[..., 2:] * mask[..., np.newaxis]
        dt_hires_std_m[..., 2:] = dt_hires_std_m[..., 2:] * mask[..., np.newaxis]
        dt_hires_std_d[..., 2:] = dt_hires_std_d[..., 2:] * mask[..., np.newaxis]
        print("Size of dt_hires after trimming: %s", (dt_hires.shape,))
    return dt_hires, dt_hires_std_m, dt_hires_std_d
